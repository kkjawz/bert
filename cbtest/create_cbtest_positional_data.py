...
class TrainingInstance(object):
    ...
    def __init__(self, tokens, segment_ids, masked_lm_position, masked_lm_label,
...
def write_instance_to_example_files(all_instances, tokenizer, max_seq_length, cbtest_vocab, output_files):
    ...
    for i, instances in enumerate(all_instances):
        for (inst_index, instance) in enumerate(instances):
            ...
            masked_lm_candidates = instance.masked_lm_candidates
...
def create_training_instances(input_files, tokenizer, max_seq_length,
        ...
        with tf.gfile.GFile(input_file, "r") as reader:
            while True:
                ...
                if int(line_index) == 21:
                    ...
                    cur_example.label = label_index
                    cur_example.candidates = candidates_index
    ...
    for cur_raw_examples in all_raw_examples:
        ...
        for document_index in range(len(cur_raw_examples)):
            cur_instances.append(
                create_instance_from_example(
...
def create_instance_from_example(
    ...
    masked_lm_label = example.label - n_truncated
    masked_lm_candidates = [c - n_truncated for c in example.candidates]

    if masked_lm_label < 0 or any([c < 0 for c in masked_lm_candidates]):
        pass
